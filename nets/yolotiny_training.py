...
class YOLOLosstiny(nn.Module):
    def __init__(self, anchors, num_classes, input_shape, cuda,
                 ...
                 label_smoothing=0, focal_loss=True, alpha=0.25, gamma=2):
    ...
    def BCELoss(self, pred, target):
        ...
        return -weight * (target * torch.log(pred) + (1.0 - target) * torch.log(1.0 - pred))
    ...
    def forward(self, l, input, targets=None):
        ...
        # 计算步长
        # 如果特征层为13x13的话，一个特征点对应32个像素点
        # 如果特征层为26x26的话，一个特征点对应16个像素点
        ...
        loss_loc = torch.mean((1 - ciou) * obj_mask * box_loss_scale)

        # 计算置信度损失
        loss_conf = torch.mean(self.BCELoss(conf, obj_mask) * (obj_mask + noobj_mask))
    ...
    def get_ignore(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w, noobj_mask):
        ...
        for b in range(bs):
            ...
            # 计算真实框，并把真实框转换成相对于特征层的大小: num_true_box, 4
            if len(targets[b]) > 0:
                batch_target = torch.zeros_like(targets[b])
                # 计算出正样本在特征层上的中心点
                batch_target[:, [0, 2]] = targets[b][:, [0, 2]] * in_w
                ...
                # 计算交并比: num_true_box, num_anchors
                anch_ious = self.calculate_iou(batch_target, pred_boxes_for_ignore)

                # 每个先验框对应真实框的最大重合度: num_anchors
